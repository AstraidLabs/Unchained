...
namespace Unchained.Application.Behaviors
{
          ...
          where TRequest : notnull
    {
        ...
        public async Task<TResponse> Handle(TRequest request, RequestHandlerDelegate<TResponse> next, CancellationToken cancellationToken)
        {
            ...
            var forceRefreshProp = typeof(TRequest).GetProperty("ForceRefresh");
            if (forceRefreshProp?.PropertyType == typeof(bool) &&
                (bool)(forceRefreshProp.GetValue(request) ?? false))
            {
                _logger.LogDebug("Skipping cache for {RequestName} due to ForceRefresh", typeof(TRequest).Name);
                return await next();
            }
