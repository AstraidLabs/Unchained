...
using Unchained.Services.Background.Events;
...
namespace Unchained.Services.Background.Services
{
    public class TokenRefreshService : BaseBackgroundService
    {
        ...
        protected override async Task ExecuteServiceAsync(CancellationToken stoppingToken)
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                await ExecuteWithEventsAsync("RefreshCheck", async () =>
                {
                    ...
                    var magentaService = scope.ServiceProvider.GetRequiredService<IUnchained>();
                    await magentaService.InitializeAsync();
        ...
        private async Task<TokenData?> RefreshTokensAsync(IUnchained service, TokenData currentTokens)
        {
            try
            {
